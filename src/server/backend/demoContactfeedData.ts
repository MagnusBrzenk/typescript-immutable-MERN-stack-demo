 ...
 */
export async function getDemoContactfeedData(): Promise<CONTACT.Interface[]> {
    return new Promise(async resolve => {
        const mockCsvData = await csv().fromFile(path.resolve("resources", "data", "MOCK_CONTACT_DATA.csv"));
        const reformattedMockCsvData = mockCsvData
            .map(
        ...
        resolve(reformattedMockCsvData);
    }) as Promise<CONTACT.Interface[]>;
}
